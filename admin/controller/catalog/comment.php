...
class ControllerCatalogComment extends Controller
{
	...
	private function getList() {
		...
		if (SC_VERSION < 22 || SC_VERSION > 22) {
			if (SC_VERSION < 20) {
               	$this->url = new Url(HTTP_CATALOG, $this->config->get('config_use_ssl') ? HTTPS_CATALOG : HTTP_CATALOG);
			} else {
		...
		if (SC_VERSION < 22 || SC_VERSION > 22) {
			if (SC_VERSION < 20) {
               	$this->url = new Url(HTTP_SERVER, $this->config->get('config_use_ssl') ? HTTPS_SERVER : HTTP_SERVER);
		} else {
