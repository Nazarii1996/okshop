...
class ModelToolExportImport extends Model {
	...
	protected function uploadCategories( &$reader, $incremental, &$available_category_ids=array() ) {
		...
		$data = $reader->getSheetByName('Export Groups Sheet');
		...
		//  echo "ne";
			...
			$parent_id = $this->getCell($data,$i,4,'0');
            if(empty($parent_id)){
                $parent_id=0;
            }
			$names = array();
...
*/
			...
			if ($exist_meta_title) {
				$category['meta_titles'] = $descriptions;
			}
			$category['meta_descriptions'] = $descriptions;
			$category['meta_keywords'] = $descriptions;
			if (!$this->use_table_seo_url) {
				$category['seo_keyword'] = $descriptions;
			}
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$sql .= "`location`,`model`,`manufacturer_id`,`image`,`shipping`,`price`,`date_added`,`date_modified`,`date_available`,`status`,";
		$sql .= "`sort_order`) VALUES ";
		$sql .= "($product_id,$quantity,'$sku','$upc',";

		$sql .= "'$location','$model',$manufacturer_id,'$image',$shipping,$price,";
		$sql .= ($date_added=='NOW()') ? "$date_added," : "'$date_added',";
		$sql .= ($date_modified=='NOW()') ? "$date_modified," : "'$date_modified',";
		...
		foreach ($languages as $language) {
			...
			if ($exist_table_product_tag) {
				if ($exist_meta_title) {
					...
					$sql .= "( $product_id, $language_id, '$name', '$description', '$name', '$description', '$meta_keyword' );";
				} else {
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$names = array();
				...
				$names[1] = $name;
	       //print_r($names);
			$categories = $this->getCell($data,$i,15,'').",";
           // echo "fasfqw";
           ...
           // echo "-";
            ...
            $meta_keywords=$this->getCell($data,$i,3,' ');
            $images=explode(",",$this->getCell($data,$i,12));
     
            
            file_put_contents(DIR_IMAGE.'catalog/products/'.$product_id.'.jpg', file_get_contents($images[0]));
            $image_name ='catalog/products/'.$product_id.'.jpg';
          
			$shipping = $this->getCell($data,$i,$j++,'yes');
			$price = $this->getCell($data,$i,6,'0.00');
