...
class ModelCatalogRecord extends Model
{
	...
	public function copyRecord($record_id) {
		...
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "record p
		...
		if ($query->num_rows) {
			...
			foreach ($results as $num => $result) {
				$data['record_image'][$num]['image'] = $result['image'];
                $data['record_image'][$num]['options'] = unserialize(base64_decode($result['options']));
			}
			...
			$data = array_merge($data, array(
				'index_page' => $this->getRecordIndexPage($record_id)
			));
	...
	public function getRecords($data = array()) {
		...
		if ($data) {
			...
			LEFT JOIN " . DB_PREFIX . $mark_category."_description bd ON (p2c.".$mark_category."_id = bd.".$mark_category."_id AND bd.language_id ='" . (int) $language_id . "')
			";
			...
			if (!$record_data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . $mark_record." p LEFT JOIN " . DB_PREFIX . $mark_record."_description pd ON (p.".$mark_record."_id = pd.".$mark_record."_id)
				WHERE pd.language_id = '" . (int) $language_id . "' ORDER BY p.".$mark_record."_id DESC");
	...
	public function getMainBlog($record_id) {
		...
		WHERE
		bd.language_id = '" . (int) $language_id . "'
		AND
		p.record_id = '" . (int) $record_id . "'
	...
	public function getRecordsByBlogId($blog_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "record p LEFT JOIN " . DB_PREFIX . "record_description pd ON (p.record_id = pd.record_id) LEFT JOIN " . DB_PREFIX . "record_to_blog p2c ON (p.record_id = p2c.record_id)
		WHERE pd.language_id = '" . (int) $language_id . "' AND p2c.blog_id = '" . (int) $blog_id . "' ORDER BY p.record_id DESC");
		return $query->rows;
	}
	...
	public function getRecordIndexPage($record_id) {
		$query = $this->db->query("SELECT index_page FROM " . DB_PREFIX . "record WHERE record_id = '" . (int) $record_id . "'");
	...
	public function getRecordAttributes($record_id) {
		...
		$record_attribute_query = $this->db->query("SELECT pa.attribute_id, ad.name FROM " . DB_PREFIX . "record_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id)
		WHERE pa.record_id = '" . (int) $record_id . "' AND ad.language_id = '" . (int) $language_id . "' GROUP BY pa.attribute_id");
