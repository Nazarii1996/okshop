  ...
  public function copyFilters($data = array()) {
    ...
    if (!empty($data['copy_option']) || !empty($data['copy_filter']) || !empty($data['copy_attribute'])) {
      ...
      if ($data['copy_status'] < 0) {
        $query = $this->db->query("SELECT oo.option_id FROM " . DB_PREFIX . "ocfilter_option oo LEFT JOIN " . DB_PREFIX . "ocfilter_option_to_category oo2c ON (oo.option_id = oo2c.option_id) WHERE oo2c.category_id IS NULL");

        foreach ($query->rows as $result) {
    	    $this->db->query("UPDATE " . DB_PREFIX . "ocfilter_option SET status = '0' WHERE option_id = '" . (int)$result['option_id'] . "'");
        }

        $query = $this->db->query("SELECT oo.option_id FROM " . DB_PREFIX . "ocfilter_option oo LEFT JOIN " . DB_PREFIX . "ocfilter_option_value_to_product oov2p ON (oo.option_id = oov2p.option_id) WHERE oov2p.product_id IS NULL");

        foreach ($query->rows as $result) {
    	    $this->db->query("UPDATE " . DB_PREFIX . "ocfilter_option SET status = '0' WHERE option_id = '" . (int)$result['option_id'] . "'");
        }

        $query = $this->db->query("SELECT oo.option_id FROM " . DB_PREFIX . "ocfilter_option oo LEFT JOIN " . DB_PREFIX . "ocfilter_option_value oov ON (oo.option_id = oov.option_id) WHERE oov.value_id IS NULL");

        foreach ($query->rows as $result) {
    	    $this->db->query("UPDATE " . DB_PREFIX . "ocfilter_option SET status = '0' WHERE option_id = '" . (int)$result['option_id'] . "'");
        }

        $query = $this->db->query("SELECT oo.option_id FROM " . DB_PREFIX . "ocfilter_option oo WHERE IF(oo.type = 'slide_dual' OR oo.type = 'slide', 0, 1) AND (SELECT COUNT(*) FROM " . DB_PREFIX . "ocfilter_option_value oov WHERE oov.option_id = oo.option_id) > '100'");
